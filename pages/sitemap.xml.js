const CATEGORIES_DATA_URL = 'https://serveradmin-whhj.onrender.com/api/categories';
const PRODUCTS_DATA_URL = 'https://serveradmin-whhj.onrender.com/api/all-products';
  ...
  <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    ${categories
      .map(({ name }) => {
        return `
      <url>
        <loc>${`https://mediewal.com.br/categories/${name}`}</loc>
      </url>
    `;
      })
      .join('')}
      ...
      .map(({ name, _id }) => {
        return `
      <url>
        <loc>${`https://mediewal.com.br/products/${formatName(name)}/${_id}`}</loc>
      </url>
    `;
      })
      .join('')}
